...
int main() {
	int x, y, maxY = 0, target_minX, target_maxX, target_minY, target_maxY, t;
	...
	for (y = 500; y > target_minY; y--) {
		for (x = 1; x <= target_maxX; x++) {
			maxY = simulate(x, y, target_minX, target_maxX, target_minY, target_maxY);
...
int simulate(int v_x, int v_y, int t_minX, int t_maxX, int t_minY, int t_maxY) {
	...
	return x <= t_maxX && y >= t_minY ? maxY : -1;
}
