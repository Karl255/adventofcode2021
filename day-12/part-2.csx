...
int[] visited = new int[nSmall];
visited[0] = 1;
WriteLine(PathsToEnd(n, adjacencyMatrix, nSmall, 0, visited, false));

int PathsToEnd(int n, bool[,] adjacency, int nSmall, int from, int[] visited, bool atOneTwice) {
	int paths = 0;
	
	if (!atOneTwice) {
		atOneTwice = visited.Contains(2);
	}
	...
	for (int i = 0; i < n; i++) {
		...
		if (i < nSmall) {
			if (visited[i] > (atOneTwice || i == 0 ? 0 : 1)) continue;
			visited[i]++;
			
			paths += PathsToEnd(n, adjacency, nSmall, i, visited, atOneTwice);
