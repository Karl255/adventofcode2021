...
outer: for (i = 0; i < drawnNumbers.length; i++) {
	for (let j = 0; j < boards.length; j++) {
		if (boards[j].draw(drawnNumbers[i])) lastWinner = boards.splice(j--, 1)[0];
		if (boards.length == 0) break outer;
	}
}

drawnNumbers = drawnNumbers.slice(0, i + 1);
console.log(lastWinner.values.filter(x => drawnNumbers.indexOf(x) === -1).reduce((x, y) => x + y, 0) * drawnNumbers[i]);
